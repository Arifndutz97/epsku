...
export default function ReadingPage() {
    ...
    const timer = setInterval(() => {
      setTimeLeft((prev) => {
        if (prev <= 1) {
          ...
          const tryoutId = pathParts[1];
          router.push(`/${tryoutId}/listening`);
          return 0;
        }
        return prev - 1;
      });
  ...
  return (
    <div className="flex h-screen font-sans">
      {/* Sidebar */}
      <div className="w-1/5 bg-gray-100 p-4 border-r overflow-auto">
                ...
                {[1, 2, 3, 4, 5].map((col) => {
                  ...
                  return (
                    <td key={col} className="p-1">
                      <button
                        className={`w-full p-2 text-sm rounded font-semibold ${currentQuestion === number
